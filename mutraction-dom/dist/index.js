...
(() => {
  ...
  function makeProxyHandler(model2, tracker2) {
    function getOrdinary(target, name, receiver) {
      if (name === TrackerOf)
        return tracker2;
      if (name === GetOriginal)
        return target;
      tracker2[RecordDependency](createOrRetrievePropRef(target, name));
      let result = Reflect.get(target, name, receiver);
      if (typeof result === "object" && !isTracked(result)) {
        const original = result;
        const handler = makeProxyHandler(original, tracker2);
        result = target[name] = new Proxy(original, handler);
        linkProxyToObject(original, result);
      }
      if (typeof result === "function" && tracker2.options.autoTransactionalize && name !== "constructor") {
        let proxyWrapped2 = function() {
          const autoTransaction = tracker2.startTransaction(original.name ?? "auto");
          try {
            const result2 = original.apply(receiver, arguments);
            if (autoTransaction.operations.length > 0) {
              tracker2.commit(autoTransaction);
            } else {
              tracker2.rollback(autoTransaction);
            }
            return result2;
          } catch (er) {
            tracker2.rollback(autoTransaction);
    ...
    function getArrayTransactionShim(target, name, receiver) {
      if (typeof name === "string" && mutatingArrayMethods.includes(name)) {
        let proxyWrapped2 = function() {
          const arrayTransaction = tracker2.startTransaction(String(name));
          const arrayResult = arrayFunction.apply(receiver, arguments);
          tracker2.commit(arrayTransaction);
          return arrayResult;
        };
        var proxyWrapped = proxyWrapped2;
    ...
    function setOrdinary(target, name, newValue, receiver) {
      if (typeof newValue === "object" && !newValue[TrackerOf]) {
        const handler = makeProxyHandler(newValue, tracker2);
      ...
      if (initialSets == setsCompleted) {
        tracker2[RecordMutation](mutation);
      }
      ++setsCompleted;
      return wasSet;
    }
    function setArray(target, name, newValue, receiver) {
      ...
      if (name === "length") {
        ...
        if (newLength < oldLength) {
          ...
          tracker2[RecordMutation](shorten);
          ++setsCompleted;
      ...
      if (isArrayIndex(name)) {
        ...
        if (index >= target.length) {
          ...
          tracker2[RecordMutation](extension);
          ++setsCompleted;
          return Reflect.set(target, name, newValue, receiver);
        }
      }
    ...
    if (Array.isArray(model2)) {
      set = setArray;
      if (tracker2.options.trackHistory)
  ...
  function track(model2, options) {
    ...
    const tracker2 = new Tracker(options);
    const proxied = new Proxy(model2, makeProxyHandler(model2, tracker2));
    linkProxyToObject(model2, proxied);
    return [proxied, tracker2];
  }
  function effect(tracker2, sideEffect, options = {}) {
    let dep = tracker2.startDependencyTrack();
    sideEffect();
    ...
    function modelChangedForEffect() {
      ...
      if (depgen === latestGen)
        return;
      latestGen = depgen;
      dep = tracker2.startDependencyTrack();
      sideEffect();
      dep.endDependencyTrack();
    }
    return tracker2.subscribe(modelChangedForEffect);
  }
  ...
  var d = element("div", { tabIndex: () => model.tab }, child(() => model.val), child(() => element("p", {}, "just something")), child(() => element("p", { classList: () => ({ stinky: model.show }) }, "touch of class")), element("p", { style: () => ({ outline: "green solid 2px" }) }, "just something unwrapped once"), child(() => element("p", { if: () => model.show }, "intermittently")), child(() => element("p", { if: () => false }, "never")), "");
